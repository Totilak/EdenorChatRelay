...
configurations {
  ...
  // include libraries
  implementation.configure { extendsFrom(library) }
  shadow.configure { extendsFrom(library) }
}
...
dependencies {
  ...
  modApi("me.shedaniel.cloth:cloth-config-fabric:19.0.147") { exclude("net.fabricmc.fabric-api") }

  library("com.github.pengrad:java-telegram-bot-api:9.2.0")
}
...
tasks.jar { from("LICENSE") { rename { "${it}_${project.base.archivesName}" } } }
...
tasks {
  ...
  shadowJar {
    ...
    dependencies { exclude { it.moduleGroup == "org.slf4j" } }
  }
}
